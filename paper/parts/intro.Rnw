...
\cc{replace: which} 
allows users to visually direct and interact with the steps of community detection. Additionally, as described
\cc{replace: I describe}
in the following, gravicom is equipped 
\cc{delete: gravicom is equipped; insert: We equipped gravicom} with several functionalities which 
\cc{replace: that}
...
We used these findings to construct a community detection visualization tool
\cc{replace: in the construction of our}. 
